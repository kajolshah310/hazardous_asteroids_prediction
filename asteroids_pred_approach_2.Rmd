...
```
```{r}
gbm_predicted = predict.gbm(gbm_model, asTesting, n.tress = 200, type = "response")
gbm_predicted
labels = colnames(gbm_predicted)[apply(gbm_predicted, 1, which.max)]
result = data.frame(asTesting$pha, labels)
#asTesting$pha = as.factor(asTesting$pha)
confusionMatrix(asTesting$pha, as.factor(labels), positive = 'Y')
```
